...
class SamenvattenInDialoog:
    ...
    def generate_assistant_response(self):
        """
        Generate a response from the assistant using streaming.
        """
        samenvatting = """
...
- **Stap 2:** mRNA wordt gespliced en bewerkt (1 punt).  
...
#### ## Leverziekten
- Leverziekten hebben invloed op de spijsvertering (1 punt) en stofwisseling (1 punt).
...
**Belangrijk:** Na elk antwoord van de student, structureer je jouw reactie als volgt:

- **student_knowledge:** [Hier noteer je de correcte kennis die de student heeft gegeven, in volledige zinnen.]
- **response:** [Hier schrijf je jouw reactie terug naar de student om het gesprek voort te zetten.]
...
"""
        # Initiating the streaming response
        response_stream = self.client.chat.completions.create(
            ...
            stream=True,  # Streaming the response
        )
    ...
    def parse_assistant_response(self, response_text):
        """
        Parse the assistant's response to extract 'student_knowledge' and 'response'.
        """
        student_knowledge = ""
        assistant_response = ""
        ...
        current_field = None
        for line in lines:
            if line.startswith("student_knowledge:"):
                current_field = "student_knowledge"
                student_knowledge = line[len("student_knowledge:") :].strip()
            elif line.startswith("response:"):
                current_field = "response"
                assistant_response = line[len("response:") :].strip()
            else:
                if current_field == "student_knowledge":
                    student_knowledge += "\n" + line.strip()
                elif current_field == "response":
                    assistant_response += "\n" + line.strip()
        return student_knowledge, assistant_response
    ...
    def handle_user_input(self):
        ...
        if user_input := st.chat_input("Jouw antwoord"):
            ...
            self.add_to_assistant_responses(assistant_response)

            # Parse the assistant's response
            student_knowledge, assistant_reply = self.parse_assistant_response(
                assistant_response
            )

            # Append 'student_knowledge' to './kennis_student.txt'
            with open("./kennis_student.txt", "a", encoding="utf-8") as f:
                f.write(student_knowledge + "\n")

            # Display 'response' to the student in the chat
    ...
    def run(self):
        ...
        if st.session_state.messages == []:
            intro_text = "We gaan samen de belangrijkste punten van het college samenvatten. Zullen we beginnen met het eerste onderwerp?"
...
if __name__ == "__main__":
    # Reset the './kennis_student.txt' file
    with open("./kennis_student.txt", "w", encoding="utf-8") as f:
        f.write("")
